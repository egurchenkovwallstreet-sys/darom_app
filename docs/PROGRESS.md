...
| | |
|---|---|
| **Текущий этап** | **C — монетизация**; **F — модерация** ✅ (Vision на сервере); **Робокасса** ⏸ (ждём одобрение) |
...
| **2** | **Yandex Vision** | ✅ на сервере (23.06.2026) | `deploy/VISION.md` |
...
| Yandex Vision (фото + OCR) | ✅ код + сервер (`vision.mock:false`) |
...
2. Приоритет основателя в ленте
3. Роль moderator
4. Android / iOS (этап D)
