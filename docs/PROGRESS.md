...
| | |
|---|---|
| **Текущий этап** | **C — монетизация**; **F — модерация** ✅ (Vision); **Sightengine** ⏳ (оружие на фото — позже); **Робокасса** ⏸ |
...
| № | Этап | Задача | Зачем |
|---|------|--------|-------|
| **1** | **C — Робокасса** ← **СЕЙЧАС** | Дождаться одобрения магазина → тест оплаты | Код ✅; правила модерации отправлены в поддержку ⏸ |
| **2** | **Sightengine** | Подключить распознавание оружия/алкоголя/табака **на фото** | ⏳ после Робокассы или по приоритету; Free 2 000 фото/мес |
| **3** | **Yandex Vision** | ✅ на сервере (23.06.2026) | `deploy/VISION.md`; **платный** (~0,1–0,5 ₽/фото) |
| **4** | Лента | Приоритет **основателя** в сортировке | Значок есть, приоритет ⏳ |
| **5** | Админка | Роль **moderator** (без денег) | Отдельные модераторы |
| **6** | **D — Магазины** | Android APK / iOS | Нативные приложения + badge на иконке |
...
### Не сделано / ждём
- **Sightengine** — распознавание оружия, алкоголя, табака **по картинке** (Yandex Vision этого не умеет); подключим позже, Free 2 000 фото/мес
...
1. **Робокасса** — после одобрения магазина ← **СЕЙЧАС** (`deploy/ROBOKASSA.md`)
