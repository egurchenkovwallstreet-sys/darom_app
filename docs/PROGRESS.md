...
| **2** | **SMTP** | Почта для кодов админа (2FA) | Код ✅ `deploy/SMTP.md`; **настроить `.env` на сервере** |
| **3** | **Firebase** | Push: бронь, чаты, «Отдал» | Код ✅ `deploy/FIREBASE.md`; **настроить Firebase + `.env`** |
| **4** | Yandex Vision ← **СЕЙЧАС** | Модерация фото | Автопроверка объявлений |
...
| Вход 2FA: **Mobile ID** (~3–6 ₽) + код на e.gurchenkov@yandex.ru | ✅ Mobile ID; ✅ SMTP (см. `deploy/SMTP.md`) |
...
| Почта SMTP для кодов админа | ✅ nodemailer; на сервере: `ADMIN_EMAIL_MOCK=false` + пароль Yandex |
...
- **Firebase push** — код ✅ (`deploy/FIREBASE.md`); настройка Firebase + `.env`
- Yandex Vision
- **Робокасса** — код ✅; URL в кабинете и `.env` готовы; **магазин на одобрении** ⏸ → тест оплаты после активации
- **SMTP** для кодов админа — код ✅ (`deploy/SMTP.md`); осталось добавить пароль в `.env` на сервере
...
4. ✅ **SMTP** — nodemailer + SMS fallback; `deploy/SMTP.md`
5. ✅ **Firebase push** — код + `deploy/FIREBASE.md`; настройка Firebase + `.env`
...
- [x] C: SMTP админ-почты (код + deploy/SMTP.md)
- [ ] D: Android / iOS
