# Техническое задание: «Даром» v3.0

> Якорный файл проекта. В новом чате: `@docs/TZ_DAROM.md` + `@docs/PROGRESS.md`

## 1. Концепция

**«Даром»** — мобильное приложение для **бесплатной** передачи вещей. Единый аккаунт: пользователь одновременно отдаёт и забирает вещи. Узкая специализация, без коммерции. Геолокация — ключевая ценность.

**Приоритет:** Flutter (Android + iOS) → потом Flutter Web.

> ⚠️ **БЕЗОПАСНОСТЬ:** этапы **I** и **J** закрыты ✅ (27.06.2026). **Публичный запуск разрешён** — оферта проверена; 2FA на панелях (GitHub/Timeweb/Reg.ru/Cloudflare) — после запуска. Чеклист: `docs/PROGRESS.md`.

## 2. Роли и монетизация

### Бесплатно всегда
- Регистрация: **номер + имя + PIN** (без SMS при регистрации)
- Карта и лента объявлений
- До **30 активных объявлений** (единый лимит для всех)
- Профиль и статистика

### Подтверждение номера (один раз навсегда)
- **Не** при регистрации — только при **первом объявлении** или **первом сообщении в чате**
- Канал: **SMS Aero Mobile ID** (~3,39–5,79 ₽): push «Подтвердить» или SMS с 4 цифрами
- После успеха — все функции без повторной проверки
- Инструкция: `deploy/MOBILE_ID.md`

### Основатель (первые 1000 пользователей)
- Значок навсегда, приоритет в ленте
- **Лимит объявлений и заборы — как у всех** (30 объявлений бесплатно; бонус заборов только если зарегистрировался по реферальному коду блогера)

### Монетизация получателей

**Бесплатные заборы в месяц** (сброс каждый календарный месяц). Порог считается по числу **активных объявлений на платформе** (статусы `active` + `reserved`):

| Режим платформы | Условие | Обычный пользователь | Реферал блогера* |
|-----------------|---------|----------------------|------------------|
| Стартовый | **&lt; 20 000** активных объявлений | **5**/мес | **7**/мес |
| Средний | **≥ 20 000** и **&lt; 50 000** | **3**/мес | **5**/мес |
| Зрелый | **≥ 50 000** активных объявлений | **2**/мес | **2**/мес |

\* **Реферал блогера** — пользователь, зарегистрировавшийся с кодом партнёра/блогера (поле `referred_by_partner_id`). Бонус **+2** забора к лимиту обычного пользователя на стартовом и среднем этапах; после 50 000 объявлений на платформе — **без бонуса** (2 для всех).

**Платные пакеты** (после исчерпания бесплатных, **каждый месяц с начала**):

| Пакет | Цена | Заборов |
|-------|------|---------|
| 1-й | **149₽** | +10 |
| 2-й | **299₽** | +10 |
| 3-й | **499₽** | +10 |
| После 3-го | **блок** до нового месяца | — |

- Цель: защита от массового бесплатного забора для перепродажи на других площадках
- Пожизненный счётчик «забрано» **не обнуляется**
- «Активировать повторно» — лимит получателя **не** тратится
- Сделка только после «Отдал» дарителем

### Монетизация дарителей
- Базово: **30** активных объявлений **для всех** (основатель и обычный пользователь — одинаково)
- После **30** активных объявлений — платная услуга **«Супер даритель»**: **99₽** за пакет **+10** активных объявлений на **30 дней** (значок, приоритет в ленте)
- При исчерпании лимита пакет можно покупать **снова без ограничений**: каждые **99₽** → ещё **+10** мест (30→40→50→…)
- Пакеты **суммируются**: лимит = 30 + (число купленных пакетов × 10)

> **Статус реализации:** backend ✅ (`limits.js`, `pickup_limits.js`), оферта ✅, UI профиля ✅ (23.06.2026).

### Оплата
Робокасса (карты, СБП, кошельки). Нужно ИП/ООО.

**Статус (27.06.2026):** ✅ **боевой режим работает** — магазин **Darom-app**, `PAYMENT_MOCK=false`, `ROBOKASSA_TEST_MODE=false`, чек 54-ФЗ (Receipt POST), e-mail в форме (`ROBOKASSA_PAYMENT_EMAIL`). Протестировано: **Супер даритель 99₽**, пакеты заборов **149/299/499₽** — тот же API. Инструкция: `deploy/ROBOKASSA.md`.

### Партнёры / блогеры (реферальная программа)

| Правило | Описание |
|---------|----------|
| Регистрация партнёра | Код **0001–1000** по очереди; кнопка «Я партнёр / блогер»; подтверждение номера — **Mobile ID** (~3–6 ₽) |
| Заявка на партнёрство | Почта **Darom.partner.ru@yandex.ru** |
| Код для аудитории | Номер партнёра (0001, 0002…) при регистрации пользователя |
| Привязка реферала | **365 дней** с регистрации |
| Комиссия | **30%** со **всех оплат** реферала в период 365 дней |
| После 365 дней | Реферал не в статистике, новые оплаты не идут |
| Выплаты | Ежемесячно; «за месяц» обнуляется после оплаты админом |
| Всего заработано | Накопительная сумма, не обнуляется |
| Статус | ✅ backend + Flutter + админ-UI «Блогеры» |

### Админ-панель

| Правило | Описание |
|---------|----------|
| URL | `/admin` (Flutter Web) — запасной путь |
| **Вход из профиля** | Пункт «Админ-панель» в ЛК — **только** если телефон есть в `admin_users` (проверка на backend: `can_access_admin_panel`) |
| **После кнопки** | Телефон уже известен → «Получить коды» → **Mobile ID** + код с почты → кабинет |
| Вход | **2FA:** **Mobile ID** (~3–6 ₽) + код на email (SMTP); запасной режим — SMS |
| Super admin | Полный доступ: жалобы, блоки, статистика, блогеры, выплаты |
| Moderator (позже) | Только жалобы и блокировки — **без** монетизации и статистики |
| Жалобы объявлений | С контекстом объявления |
| Жалобы чатов | Полная переписка; кнопка «Пожаловаться на чат» в каждом чате |
| Блокировка | Временная **1–7 дней** (ручной выбор) или **навсегда** |
| Статистика | Пользователи, активные объявления, оплаты — периоды: день/неделя/месяц/всего |
| Блогеры | Следующий код (копировать), по каждому: телефон, рефералы, суммы, кнопка «Оплатить» |
| SMTP кодов | ✅ боевой (Yandex SMTP 587) + SMS fallback; `deploy/SMTP.md` |
| Статус | ✅ backend + Flutter UI + вход из профиля |

## 3. Технологический стек

| Компонент | Технология | Статус |
|-----------|------------|--------|
| Mobile | Flutter | ✅ UI + Web в продакшене |
| Backend | Node.js + Express | ✅ PM2 на Timeweb |
| БД | PostgreSQL + PostGIS | ✅ Docker, порт 5433 |
| Фото | Yandex Object Storage | ✅ на сервере (`s3Ready: true`) |
| Чат / Push | Firebase FCM (Web: `push_helper.js` + SW) | ✅ боевой; iPhone PWA ✅; `deploy/FIREBASE.md` |
| Карты | flutter_map + OpenStreetMap | ✅ бесплатно |
| Модерация | Yandex Vision + стоп-слова + запрещённые товары + **Sightengine** ⏳ | Vision ✅ (`deploy/VISION.md`); Sightengine — оружие на фото, позже |
| SMS | SMS Aero + Mobile ID | ✅ боевой (`SMS_MOCK=false`, `SMS_AUTH_MODE=mobile_id`) |
| Оплата | Робокасса | ✅ боевой 27.06.2026; `deploy/ROBOKASSA.md` |

## 4. Категории

Одежда, **Для дома** (мебель по комнатам, посуда), Детское, Электроника, Книги, Спорт, **Строй материалы**, Другое, **Прочее** — см. `lib/data/app_categories.dart`

## 5. Визуальный стиль Midnight Glow

**Цвета:** `#001F3F`, `#008C8C`, `#00BFFF`, белый, `#FFC107`, `#FF5722`

**Фон:** `MidnightGlowScreen` — планета `earth.png` **первой**, fade 1с + scale 1.0→1.2 за 12с, градиент, размытые пятна, декорации.

**Режим `lightweight` (16.07.2026):** при старте приложения — упрощённый фон без тяжёлой анимации планеты (быстрее первый кадр на Web).

**Кнопки:** scale 1.08 / 150ms, shimmer 2s.

**Реализация:** `lib/widgets/midnight_glow_screen.dart`, `lib/theme/app_colors.dart`

## 6. Экраны

| Экран | Статус |
|-------|--------|
| Онбординг (3 слайда) + «Я партнёр» | ✅ |
| Телефон + PIN; регистрация **без SMS**; подтверждение номера **один раз** при активности (Mobile ID) | ✅ |
| Создание профиля (имя + код блогера?) | ✅ + API |
| Регистрация партнёра + заявка на почту | ✅ Mobile ID при регистрации |
| Статистика партнёра (2 суммы выплат) | ✅ |
| Главная (карта, категории, счётчики) | ✅ |
| Подкатегории | ✅ |
| Лента объявлений | ✅ API |
| Карточка объявления | ✅ + бронь, жалоба, рейтинг |
| Создание объявления | ✅ + API, стоп-слова, запрещённые товары, лимиты |
| Профиль | ✅ + основатель, партнёр, заборы, **5 уровней дарителя (бейджи)**, **Уведомления (push)**, **Служба поддержки**, **жалобы/обращения (админ — в профиле)**, **админ-панель (статистика/блогеры)** |
| Полноэкранная карта | ✅ + кнопки радиуса на карте |
| Мои объявления | ✅ |
| Бронирование 24 ч (база: API, карточка объявления) | ✅ |
| **Сделка через чат** (условия брони, подсказка дарителю, кнопки в чате) | ✅ см. §7.2 |
| Чаты + непрочитанные + жалоба на чат | ✅; push ✅ протестировано |
| Админ-панель (2FA Mobile ID + email, жалобы, блоки, stats, блогеры) | ✅ + вход из профиля |
| Публичная оферта (тарифы, условия, **модерация п. 10.8**) | ✅ `/offer`, `lib/data/public_offer.dart` |
| Оплата (Робокасса) | ✅ боевой 27.06.2026; 99₽ + пакеты заборов |

## 7. Системные правила

- **Модерация (текст):** стоп-слова ✅ + фильтр запрещённых товаров ✅ (`prohibited_goods.js`); 3 жалобы → скрытие ✅
- **Модерация (фото):** Yandex Vision — moderation + OCR ✅ на сервере (**платный**, ~0,1–0,5 ₽ за фото); **Sightengine ⏳** — оружие, алкоголь, табак по картинке (подключим позже; Free 2 000 фото/мес)
- **Запрещено:** товары, запрещённые законом РФ; товары, требующие лицензии (лекарства, алкоголь, табак, оружие, пиротехника и др.) — см. оферта п. 10.8
- **Разрешённые категории:** Одежда, Для дома, Детское, Электроника, Книги, Спорт, Строй материалы, Другое — только **бытовые вещи**, без коммерции
- **Рейтинг:** 1–5 после сделки ✅; <4.0 → теневой бан ✅ (backend)
- **Лимиты:** объявления **30** + «Супер даритель» (+10 за 99₽) ✅; заборы **5/7 → 3/5 → 2** + пакеты **149→299→499₽** ✅
- **Клавиатура на Web:** поля ввода не перекрываются (`KeyboardInsetPadding` / `AuthFormScroll`) ✅
- **Безопасность:** 1 телефон = 1 аккаунт ✅; PIN 4 цифры ✅; подтверждение реального номера один раз (Mobile ID) ✅; предупреждение о номере в чате ✅; блокировка пользователей ✅; админ — 2FA (Mobile ID + **почта** ✅) ✅
- **Фото:** Yandex S3, отдача через API `/api/photos/listings/`; nginx `location ^~ /api/` ✅
- **Геолокация Web:** запрос на HTTPS (darom-app.online) ✅; подсказки и «Повторить» при отказе ✅; таймаут 6 с, без блокировки старта ✅ (16.07)
- **Производительность Web:** ленивые вкладки, отложенные API, splash в `index.html`, gzip nginx ✅ (16.07)
- **Чаты:** в списке только диалоги с ≥1 сообщением (пустые не показываются) ✅
- **Забронировано (база):** 24ч, серый статус ✅; push дарителю ✅; «Отдал» / «Активировать повторно» на **карточке объявления** ✅
- **Сделка через чат (§7.2):** ✅ 27.06.2026 — бронь после переписки; системное сообщение; кнопки дарителя в чате

### 7.2 Сделка через чат (бронирование) — актуальная логика (27.06.2026)

> **Статус:** ✅ реализовано (backend + Flutter Web, 27.06.2026). Миграция: `migrate_chat_system_messages.sql`.

**Цель:** договориться в чате → забронировать → честно завершить сделку; защита от обхода лимита заборов (удаление объявления без «Отдал»).

| Шаг | Кто | Что происходит |
|-----|-----|----------------|
| 1 | Получатель | Пишет **первое сообщение** автору в чате по объявлению |
| 2 | Даритель (автор) | **Отвечает** хотя бы одним сообщением |
| 3 | Получатель | Становится активной кнопка **«Забронировать на 24 ч»** (до шагов 1–2 — **неактивна**) |
| 4 | Получатель | Нажимает «Забронировать» → объявление `reserved`, бронь 24 ч |
| 5 | Даритель | В чате появляется **системное сообщение** (точный текст): *«Вещь забронирована. После передачи обязательно отметьте, что сделка прошла успешно — так вам засчитается отданная вещь в рейтинг»* |
| 6 | Даритель | В **этом же чате** — две кнопки: **«Отдал вещь»** и **«Активировать повторно»** (дублируют логику карточки объявления) |
| 7a | Даритель → «Отдал вещь» | Сделка засчитана: дарителю +1 «отдано» / рейтинг; получателю +1 «забрано» и **−1 забор** (бесплатный или из пакета) |
| 7b | Даритель → «Активировать повторно» | Объявление снова **active**; забор получателя **не** списывается (как в §2) |

**Правила:**
- Сделка считается состоявшейся **только** после «Отдал вещь» (не от брони и не от удаления объявления).
- «Активировать повторно» — если вещь **не** передана (не приехали / отказ); защита от сценария «забронировали → удалили объявление → забор не списался».
- Push при брони дарителю ✅ (как сейчас).

### 7.3 Push-уведомления (Firebase FCM, Flutter Web) — ✅ 28.06.2026

> **Статус:** боевой проект **darom-6509d**; push при **брони**, **чате**, **«Отдал»**; включение на Web **протестировано** (iPhone, PWA). Инструкция: `deploy/FIREBASE.md`.

**События (backend `push_service.js`):**

| Событие | Кому | Пример текста |
|---------|------|---------------|
| Бронь 24 ч | Дарителю | «Иван забронировал(а) „Куртка“ на 24 ч» |
| Новое сообщение | Собеседнику | «Мария · Куртка: Заберу завтра» |
| «Отдал» | Получателю | «Даритель отметил „Отдал“ — оцените сделку» |

**Включение push на Web (UX):**

| Шаг | Поведение |
|-----|-----------|
| 1 | После входа — диалог **«Уведомления»** с кнопками **«Включить»** / **«Не сейчас»** |
| 2 | По **«Включить»** — системный запрос браузера «Разрешить уведомления?» (жест пользователя обязателен) |
| 3 | Повторно: **Профиль → Уведомления** |
| 4 | FCM-токен сохраняется: `POST /api/users/push-token` (Bearer) → таблица `user_push_tokens` |

**Техническая реализация Web (важно для поддержки):**

- **`web/push_helper.js`** — разрешение + получение FCM-токена в чистом JavaScript; ответ в Dart — **JSON-строка** (избегает TypeError JS-interop).
- **`web/firebase-messaging-sw.js`** — фоновые push; конфиг с `/api/config/firebase`; init на `install`.
- **`lib/services/push_service.dart`** — на Web **не** вызывает `FirebaseMessaging.requestPermission` / modular `getToken` (конфликт с compat SW).
- nginx: `notifications=(self)` в `deploy/nginx-security-headers.conf`.

**iPhone / Safari:**

- Push на iOS Web: **iOS 16.4+**, сайт добавлен **«На экран Домой»** (PWA); в обычной вкладке Safari — часто **не работает** (ограничение Apple).
- Красный badge на иконке PWA — ⏳ не гарантируется на iOS Web.

**Файлы:** `lib/services/push_service.dart`, `push_service_web.dart`, `lib/widgets/push_permission_dialog.dart`, `lib/screens/main_shell.dart`, `web/push_helper.js`, `web/firebase-messaging-sw.js`.

### 7.1 Авторизация (актуальная логика, 21.06.2026)

| Этап | Что происходит | SMS / стоимость |
|------|----------------|-----------------|
| **Регистрация** | Номер → имя → PIN (4 цифры) | **Нет** |
| **Вход** | Номер + PIN | **Нет** |
| **Первое объявление или 1-е сообщение в чате** | Диалог «Подтвердить номер»; Mobile ID | **~3–6 ₽** (SMS Aero) |
| **Партнёр** | Код + телефон → Mobile ID → имя → PIN | **~3–6 ₽** (Mobile ID) |
| **Админ-панель** | Mobile ID + код с **почты** | **~3–6 ₽** (Mobile ID) + **письмо на email** ✅ |
| **Сброс PIN** | SMS-код | Реальное SMS |

Поля в БД: `phone_verified_at` (аккаунт создан), `real_phone_verified_at` (номер подтверждён навсегда).

## 8. Уровни дарителей

Новичок → Активный (5+, рейтинг 4.0+) → Щедрый (20+) → Благотворитель (50+) → **Самое доброе сердце** (100+, 4.8+)

**В профиле (UI):** блок «🏆 Достижения» — **5 бейджей** по этим уровням (`lib/data/profile_achievements.dart`). Не полученный уровень — **маленький и серый**; полученный — **большой и цветной**. Счётчик: получено / 5. Логика совпадает с backend (`donor_level.js`).

## 9. Продакшен и домен (этап B+)

| Компонент | Значение |
|-----------|----------|
| Домен | **darom-app.online** (Reg.ru) |
| Сайт | https://darom-app.online/ |
| API | https://darom-app.online/api/ (nginx proxy, без :3000) |
| Запасной IP | http://5.129.243.246/ |
| SSL | Let's Encrypt (certbot, автопродление) |
| **Gzip (Web)** | JS + WASM: ~10,7 МБ → **~3,9 МБ**; `deploy/nginx-gzip.conf`, `deploy/NGINX_GZIP.md` ✅ 16.07 |
| Иконка | белая **D** + бирюза, `assets/icon/app_icon.png`, PWA + Android |
| Название приложения | **Даром** |

Инструкция: `deploy/DOMAIN_HTTPS.md`

### Производительность Flutter Web (16.07.2026) ✅

| Мера | Описание |
|------|----------|
| **Gzip nginx** | `main.dart.js`, `canvaskit.wasm` и др. — `Content-Encoding: gzip` |
| **Ленивые вкладки** | `MainShell` строит вкладку только при первом открытии |
| **Отложенные API** | Чаты, избранное, профиль — запросы при активации вкладки |
| **Неблокирующий старт** | Миграции и геолокация не задерживают `runApp()` |
| **Splash `index.html`** | Спиннер + статус; скрытие после появления `flutter-view` |
| **Midnight Glow lightweight** | Облегчённый фон при загрузке |

**Проверка gzip с ПК:**
```powershell
curl.exe -sSI -H "Accept-Encoding: gzip" "https://darom-app.online/main.dart.js"
```
Ожидается строка `Content-Encoding: gzip`.

**На сервере:** в `/etc/nginx/nginx.conf` уже есть `gzip on;` — в `conf.d` копировать конфиг **без** этой строки (см. `deploy/NGINX_GZIP.md`).

### DNS, Cloudflare и доступ из РФ (17.07.2026)

> **Важно:** периодические сбои «сайт не открывается без VPN» при **верных** настройках — это **DNS/сеть**, не баг Flutter/backend. Подробности: `docs/PROGRESS.md` → резюме **17.07.2026**.

**Эталон (Reg.ru):**
```
kira.ns.cloudflare.com
weston.ns.cloudflare.com
```

**Эталон (Cloudflare → DNS → Records):**

| Type | Name | Content | Proxy |
|------|------|---------|-------|
| A | `@` | `5.129.243.246` | **DNS only** (серое ☁️) |
| A | `www` | `5.129.243.246` | **DNS only** (серое ☁️) |

**Статус домена:** **Active** (не Moved / not active yet).

**Письмо Cloudflare «Moved»:** Cloudflare временно не видит NS → проверить Reg.ru → **Check nameservers now** в панели. Жёлтый баннер «Enable Proxy» — **не включать** (для РФ без VPN).

**Типичные причины нестабильности:**

| Причина | Признак |
|---------|---------|
| Cloudflare **Moved** | В панели не Active; разные DNS дают разный ответ |
| DNS провайдера РФ | `nslookup` без 8.8.8.8 падает; с 8.8.8.8 — OK |
| Фильтрация HTTPS | На сервере `curl` → 200; с телефона — таймаут |
| VPN «лечит» | Другой DNS и маршрут, не «починка сервера» |

**Быстрая диагностика (ПК):**
```powershell
nslookup darom-app.online 8.8.8.8
curl.exe -sSI "https://darom-app.online/"
```
Ожидание: IP **`5.129.243.246`** (не `104.21.x.x`), HTTP **200 OK**.

Инструкция: `deploy/CLOUDFLARE.md`

### Публичная оферта

- Текст в приложении: **«Принимаю условия оферты»** при регистрации; экран `/offer`
- Файл: `lib/data/public_offer.dart`
- Содержит: бесплатные услуги, тарифы (99₽ / 149→299→499₽), правила сервиса, **раздел 10.8 — модерация и запрещённые категории**, реквизиты ИП
- Правила модерации также переданы в поддержку Робокассы (23.06.2026)
- Актуализировать при изменении цен, правил монетизации или модерации

## 10. План разработки

1. ✅ UI + стиль + навигация
2. ✅ Backend + PostgreSQL/PostGIS + API
3. ✅ SMS API (SMS Aero + Mobile ID; регистрация без SMS; PIN ✅)
4. ✅ Карта — flutter_map + OpenStreetMap
5. ✅ Чаты ✅; Firebase push ✅
6. 🟡 Модерация: стоп-слова ✅ + запрещённые товары ✅ + оферта 10.8 ✅; Vision ✅; Sightengine ⏳
7. 🟡 Лимиты и диалоги ✅; Робокасса ✅ (27.06); сделка через чат ✅
8. ✅ **Партнёры / блогеры**
9. ✅ **Админ-панель** (2FA, жалобы, блоки, stats, блогеры, **вход из профиля**)
10. ✅ **Домен + HTTPS** (darom-app.online)
11. ✅ **Flutter Web в продакшене** (GitHub Actions ✅, геолокация HTTPS ✅, иконка ✅, **ускорение загрузки + gzip** ✅ 16.07)
12. ⏳ Публикация в магазины (Android / iOS)

**Оценка прогресса (16.07.2026):** ядро MVP ~**99%** | полное ТЗ ~**83%** | **готов к публичному запуску** 🟢. Подробности: `docs/PROGRESS.md`.

## 12. Следующие этапы (приоритет)

| Этап | Задачи | Статус |
|------|--------|--------|
| **I — Безопасность** | I-A…I-F + Cloudflare DNS only + Timeweb DDoS ✅ (26.06) | ✅ **закрыт** |
| **J — Глубокий аудит** | J-A…J-G ✅ (27.06); daily backup cron ✅ | ✅ **закрыт** |
| **C — Монетизация** | Робокасса ✅ (27.06); SMS Aero ✅; Mobile ID ✅; SMTP ✅; Firebase push ✅ | ✅ |
| **K — Сделка в чате** | Переписка → бронь → кнопки в чате | ✅ |
| **Запуск** | Оферта ✅; бэкапы cron; 2FA панелей — после запуска | 🟢 **готов** |
| **D — Магазины** | Android APK / iOS | ⏳ |
| **E — Уведомления** | Firebase push; Web: диалог + `push_helper.js`; iPhone PWA ✅ | ✅ **протестировано** (28.06.2026) |
| **F — Модерация** | Vision ✅; **Sightengine** (оружие на фото) ⏳ | 🟡 |
| **G — Админка** | Роль moderator | ⏳ |
| **H — Лента** | Приоритет основателя | ✅ |
| **Web perf** | Ленивые вкладки, splash, gzip nginx | ✅ **16.07.2026** |
| **Infra DNS** | Reg.ru NS + Cloudflare Active + DNS only; нестабильность без VPN — диагностика | ✅ перепроверено **17.07.2026** |

### 12.1 Экономическая модель (ориентир для владельца, 27.06.2026)

> **Не финансовый прогноз.** Упрощённые допущения для планирования. Реальность зависит от конверсии в оплату и активности.

**Доход (месяц):** «Супер даритель» **99₽** + пакеты заборов **149/299/499₽** (макс. **947₽**/получатель/мес).  
**Расход:** VPS, S3, Vision (~**0,3 ₽/фото**), Mobile ID (~**5 ₽**/подтверждение), комиссия Робокассы (~**3,5%**), выплаты блогерам (**30%** с оплат рефералов 365 дней).

| Зарегистр. пользователей | MAU* | Доход (низкий…высокий)** | Расход инфра*** | Чистый ориентир |
|--------------------------|------|--------------------------|-----------------|-----------------|
| **10 000** | ~3 500 | **9…35 тыс ₽** | **6…12 тыс ₽** | **−3…+25 тыс ₽** |
| **100 000** | ~30 000 | **90…350 тыс ₽** | **35…80 тыс ₽** | **+40…270 тыс ₽** |
| **1 000 000** | ~250 000 | **0,8…3,5 млн ₽** | **0,3…1,2 млн ₽** | **+0,4…2,5 млн ₽** |
| **10 000 000** | ~2 000 000 | **7…28 млн ₽** | **3…12 млн ₽** | **+3…20 млн ₽** |

\* MAU ≈ 20–35% от базы (снижается при росте).  
\** Низкий: ~0,3% MAU платят «Супер даритель», ~1% MAU — пакеты (~220 ₽ ср.); высокий: ~1% + ~3%. Без учёта НДС/налога ИП.  
\*** VPS масштабируется; при 1M+ — отдельный сервер БД, CDN, мониторинг. Партнёрские **30%** включайте отдельно (~10–40% валового дохода при сильных блогерах).

Подробная расшифровка: `docs/PROGRESS.md` → «Экономика».

Резюме сессии 16–21.06 и детальный чеклист: `docs/PROGRESS.md` → «Резюме» и «Следующие шаги».

## 13. Безопасность ✅ (этапы I + J закрыты 27.06.2026)

> **Статус:** этапы **I** и **J** выполнены; сайт **запущен для пользователей** 🟢. Поддерживать: curl §13.4 ежемесячно, бэкапы cron, 2FA на панелях — по возможности.

### 13.1 Что уже хорошо ✅

- HTTPS + редирект на HTTPS (darom-app.online)
- **Сессионные токены после PIN** — защищённые API только с `Authorization: Bearer` (I-A, 26.06)
- **Закрыт** публичный `GET /api/partners/next-code` — код только в админке (I-B, 26.06)
- **Rate limit** — PIN/SMS/админ (I-B); **общий API** 100/min + auth 20/min (I-F, 26.06)
- **CORS** — только `darom-app.online` + `localhost:8080` (I-B, 26.06)
- **Webhook Mobile ID** — секрет в URL (`MOBILE_ID_WEBHOOK_SECRET`, I-B)
- **nginx:** HSTS, CSP, X-Frame, nosniff (I-D, 26.06) — `deploy/nginx-security-headers.conf`
- **nginx gzip (Web):** сжатие JS/WASM — ~3× меньше трафика (16.07) — `deploy/nginx-gzip.conf`, `deploy/NGINX_GZIP.md`
- **Cloudflare:** DNS на NS Cloudflare, записи **DNS only** (серое ☁️) — сайт в РФ **без VPN** (I-E, 26.06); `deploy/CLOUDFLARE.md`
- **Timeweb DDoS** — «Защита от DDoS» включена на VPS (I-E, 26.06)
- **Деплой backend** — VNC (`git pull` + pm2) ✅; GitHub Actions — опционально
- Боевой `.env` на сервере (mock выключены, I-C); legacy `ADMIN_SECRET` убран
- **J-B (27.06):** payment status — только свой заказ; active-verify — только с Bearer
- **J-C (27.06):** check-phone без `user_name`; PIN lockout 5→15 мин; logout на сервере
- **J-D (27.06):** webhook Mobile ID strict; Robokassa callback idempotent
- **J-E (27.06):** sanitize HTML в чатах/объявлениях; фото nosniff; health без имени bucket
- **Observatory B+** (80/100) — nginx + Flutter Web CSP (27.06)
- Админ-панель: Mobile ID + код с почты; API админки **без токена не пускает**
- PIN **захеширован** (pbkdf2); блокировка пользователя на защищённых маршрутах
- Робокасса: проверка подписи callback (в коде)
- Фото через API `/api/photos/`, не прямой публичный S3
- Теневой бан &lt;4.0; жалобы 3→скрытие объявления
- **Flutter dev (:8080):** API → `https://darom-app.online` (`api_config.dart`, 26.06)

### 13.2 Аудит 24.06.2026 — статус исправлений

| Приоритет | Уязвимость | Статус |
|-----------|------------|--------|
| 🔴 P0 | API без токена после PIN | ✅ **I-A** (26.06) |
| 🔴 P0 | `GET /api/users?phone=` без авторизации | ✅ **I-A** |
| 🔴 P0 | `GET /api/partners/next-code` публично | ✅ **I-B** |
| 🔴 P0 | `GET /api/payments/status` — чужой inv_id | ✅ **J-B** (27.06) |
| 🔴 P0 | `active-verify/*` без Bearer — захват аккаунта | ✅ **J-B** (27.06) |
| 🔴 P0 | Robokassa повторный callback — двойное начисление | ✅ **J-D** (27.06) |
| 🔴 P1 | Webhook Mobile ID без секрета | ✅ **I-B** + **J-D** (strict) |
| 🔴 P1 | PIN без rate limit | ✅ **I-B** + **J-C** (IP + аккаунт) |
| 🟠 P1 | check-phone enumeration (user_name) | ✅ **J-C** |
| 🟠 P2 | verify-code / partner-code brute force | ✅ **J-C** (rate limit) |
| 🔴 P1 | Блок пользователя не на всех API | ✅ **I-A** (middleware) |
| 🟠 P2 | CORS `*` | ✅ **I-B** |
| 🟠 P2 | `PAYMENT_MOCK`, mock админ-почты | ✅ **I-C** (26.06) |
| 🟠 P2 | Legacy `ADMIN_SECRET` | ✅ **I-C** (26.06) |
| 🟠 P2 | Общий rate limit API | ✅ **I-F** (26.06) |
| 🟡 P3 | nginx: HSTS, CSP, X-Frame, nosniff | ✅ **I-D** (26.06) |
| 🟡 P3 | health → имя S3 bucket | ✅ **J-E** (27.06) |
| 🟡 P3 | XSS в чатах/объявлениях | ✅ **J-E** (`sanitize_text.js`) |
| 🔵 Infra | DDoS / Cloudflare | ✅ **I-E** — DNS only + Timeweb DDoS (26.06) |
| 🔵 Infra | Бэкап БД pg_dump | ✅ **J-F** (`daily_db_backup.sh`, cron 03:00, 15 дней) |

### 13.2.1 Детали уязвимостей (справочно, аудит 24.06)

| Приоритет | Уязвимость | Как закрыли |
|-----------|------------|-------------|
| 🔴 P0 | Утечка профиля по номеру | Токен + 401 |
| 🔴 P0 | Утечка кода партнёра | Endpoint → 403; код в `/api/admin/partner-codes/status` |
| 🔴 P1 | Подделка webhook | `MOBILE_ID_WEBHOOK_SECRET` в callback URL |
| 🔴 P1 | Брутфорс PIN | express-rate-limit 5/15 мин |
| 🟠 P2 | CORS `*` | whitelist origin в `index.js` |
| 🟡 P3 | Observatory −65 | nginx HSTS/CSP ✅ (I-D) |

### 13.3 DDoS — отдельно от кода приложения

DDoS **не останавливается** только кодом Node.js. Реализовано (26.06.2026):

| Слой | Статус |
|------|--------|
| **Timeweb DDoS** | ✅ «Защита от DDoS» на VPS |
| **Cloudflare** | ✅ DNS (NS Cloudflare); записи **DNS only** — для РФ **не Proxied** (иначе провайдеры режут трафик) |
| **nginx** | ✅ HSTS, CSP, лимит body 20M |
| **Backend** | ✅ rate limit 100 req/min (I-F) |

> **Россия:** оранжевое облако Cloudflare (Proxied) с июня 2025 часто **не открывается без VPN**. Для «Даром» — только **серое облако** (DNS only). См. `deploy/CLOUDFLARE.md`.

> **Нестабильность (17.07.2026):** даже при сером облаке сайт **иногда** недоступен без VPN — Cloudflare статус **Moved**, DNS провайдера или фильтрация HTTPS до Timeweb. Не менять код — см. §9 «DNS, Cloudflare и доступ из РФ» и `docs/PROGRESS.md` (17.07).

### 13.4 Обязательная проверка перед запуском для всех

Повторять **после каждого** крупного деплоя безопасности:

```powershell
# 0) security.stage:"J-G" (или новее), textSanitization:true, pinAccountLockout:true, dailyDbBackup:true
curl.exe "https://darom-app.online/api/health"

# DNS — должен быть IP Timeweb, не Cloudflare 104.21.x.x
nslookup darom-app.online 8.8.8.8

# Доступ без VPN (если жалобы): curl с ПК + сайт с телефона в приватной вкладке
curl.exe -sSI "https://darom-app.online/"
# На сервере (VNC): curl -sSI https://darom-app.online/ | head -3
# Cloudflare: Overview = Active; DNS Records = серое облако (DNS only)

# 1) Профиль без токена — 401, НЕ JSON с данными
curl.exe "https://darom-app.online/api/users?phone=НОМЕР"

# 2) Код партнёра — 403, НЕ {"code":"0007"}
curl.exe "https://darom-app.online/api/partners/next-code"

# 3) Админка — «Нужен вход в админ-панель»
curl.exe "https://darom-app.online/api/admin/stats/platform?period=day"
```

Плюс: [Mozilla Observatory](https://observatory.mozilla.org) — цель **B+** после nginx-заголовков.

**Пошаговый план реализации:** `docs/PROGRESS.md` → «План реализации защиты (Этап I)».

## 11. Структура кода (актуальная)

```
lib/
  main.dart
  data/         app_categories.dart, public_offer.dart, profile_achievements.dart, map_radius_options.dart
  models/       user.dart (+ canAccessAdminPanel), listing.dart, ...
  services/     api_config, auth_api, push_service.dart, push_service_web.dart, ...
  widgets/      push_permission_dialog.dart, real_phone_verify_dialog.dart, midnight_glow_screen, ...
web/            push_helper.js, firebase-messaging-sw.js, index.html
backend/
  src/routes/   auth.js, users.js, config.js (/api/config/firebase), admin.js, deploy_backend.js, …
  src/services/ push_service.js (FCM send)
  src/middleware/ user_auth.js, rate_limit.js, mobile_id_webhook.js
  src/security_version.js   — метка J-E в /api/health
  scripts/      deploy_backend.sh
  db/           migrate_push_tokens.sql, migrate_user_sessions.sql, …
.github/workflows/  deploy-web.yml, deploy-backend.yml
deploy/         FIREBASE.md, nginx-security-headers.conf
```
