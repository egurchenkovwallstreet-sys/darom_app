...
**Запасной вариант (уже в коде):** если оба порта SMTP недоступны, backend **автоматически** отправит **6 цифр «с почты»** обычным **SMS** на admin-номер (`ADMIN_PHONE`) через SMS Aero. В админке появится подсказка «код отправлен SMS». Mobile ID по-прежнему отдельно на телефон.
